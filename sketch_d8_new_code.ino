...
bool sendSmsWithRetry(const char* target, const String& message) {
  for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (attempt < MAX_RETRIES) {
      ...
      if (!initModem()) {
        //new code for the d8 only since changing the characteristics
          SerialMon.println("================================");
          SerialMon.println("A7670C INITIALIZATION FAILED");
          SerialMon.println("NO ESP32 RESTART");
          SerialMon.println("CONTINUING TO SENSOR/SMS CYCLE");
          SerialMon.println("================================");

        //below is the code original code
...
void loop() {
  ...
  // 1. Properly shut down the modem via AT command
  SerialMon.println("Powering down modem gracefully...");
  SerialAT.println("AT+CPOWD=1"); // Software power down command
  delay(2000); // Give the modem time to log off the network
  
  // 2. Turn off all sensors to cut power consumption
  digitalWrite(All_5V_SENSOR, LOW);
  
  SerialMon.println("Entering deep sleep for 10 minutes...");
  delay(100); // Small delay to let serial finish printing
  
  // Flush streams
  SerialAT.flush();
  SerialMon.flush();
  
  // Enter deep sleep safely
